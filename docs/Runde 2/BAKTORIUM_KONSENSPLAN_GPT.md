# Baktorium – Planungsdokument v0.2
...
> Status: **nicht final** – v0.2 integriert die Gegenpläne von Claude Code und Codex als Konsens-/Delta-Fassung.
...
## 6. Konkreter Dateivorschlag für den Neustart – v0.2

Die Gegenpläne sprechen dafür, die Codebasis direkt unter `src/` zu bündeln und Doku/Resources/Scenes klar daneben zu halten. Dadurch bleibt der Godot-Projektroot lesbar und Agenten finden schneller, ob sie Code, Doku, Ressourcen oder Szenen bearbeiten.

Startstruktur:

```text
AGENTS.md
CLAUDE.md                  # sehr kurz; Claude-spezifische Review-Brücke, kein zweiter Vertrag
README.md
project.godot

docs/
  BAKTORIUM_NEUSTART_CANVAS.md
  ARCHITEKTUR.md
  SIM_RULES.md
  DECISIONS.md
  STATUS.md
...
- `src/core/` bleibt Godot-Node-frei.
- `src/body/` hält lokale Körperwahrheit.
- `src/sim/` hält autoritative Tick-, Energie-, Wachstums- und Weltlogik.
- `src/runtime/` erzeugt abgeleitete Snapshots.
- `src/rendering/`, `src/ui/`, `src/debug/` sind Projektion und Diagnose.
- `resources/config/*.tres` sind editierbare Parameter, aber keine Runtime-Wahrheit.
- `resources/cell_functions/*.tres` definieren Zellfunktionen als Daten-Tags, nicht als Klassenhierarchie.

Autoload-Regel v0.2:

- Keine verpflichtenden Autoloads in Slice 0.
- Wenn Autoloads eingeführt werden, nur nach ADR und nur für echte globale Infrastruktur.
- Kandidaten wären maximal `TimeService` und/oder `SimRegistry`; bevorzugt bleibt zunächst die explizite Verdrahtung über die Lab-Composition-Root.

---

## 7. Dokumentationssystem – schlank, aber wirksam

### 7.1 Kanonische Dokumente v0.2

Die Doku soll die Agenten führen, aber nicht selbst zum Projektmonster werden. Der Konsens aus den Gegenplänen ist: **wenige kanonische Dokumente, klare Zuständigkeit, keine historischen Handoff-Müllhalden.**

| Datei | Funktion |
|---|---|
| `AGENTS.md` | Hauptvertrag für Codex, Claude Code und ChatGPT |
| `CLAUDE.md` | sehr kurze Claude-Code-Review-Brücke; verweist auf `AGENTS.md`, ersetzt ihn nicht |
| `README.md` | Kurzstart, Projektzweck, Tests, wichtigste Befehle |
| `docs/BAKTORIUM_NEUSTART_CANVAS.md` | offenes Planungs- und Kalibrierungsdokument |
| `docs/ARCHITEKTUR.md` | normative Schichten, Autoritäten, Verbote, Autoload-Regeln |
| `docs/SIM_RULES.md` | Hex-Koordinaten, Zellfunktionen, Energie, Wachstum, Genom, Tick-Reihenfolge, Nicht-Ziele |
| `docs/DECISIONS.md` | ADRs für echte Richtungsentscheidungen |
| `docs/STATUS.md` | aktueller realer Stand: implementiert, sichtbar, validiert, offen |
| `docs/NEXT_STEPS.md` | genau der nächste Arbeitsblock mit Gate und Nicht-Zielen |
| `docs/FINDINGS.md` | Bugs, Debug-Befunde, Review-Findings und geplante Korrekturen |

Änderung gegenüber v0.1:

- `CELL_SYSTEM.md` geht in `SIM_RULES.md` auf, weil Zellregeln nicht isoliert von Energie, Wachstum, Genom und Tick-Reihenfolge betrachtet werden sollten.
- `BUGS.md` wird durch `FINDINGS.md` ersetzt, weil nicht jeder Befund ein Bug ist. Review-Hinweise, Architektur-Risiken und Debug-Erkenntnisse brauchen denselben Eingangskanal.
- `CLAUDE.md` bleibt erlaubt, aber nur als kurzer Review-Fokus. Kein zweites Regelwerk.

Nicht empfohlen für den Start:

- großes `HANDOFF.md`
- langes historisches `AI_KONTEXT.md`
- separate `PERFORMANCE.md`
- Probe-/Run-Archive ohne echten Bedarf
...
3. `docs/SIM_RULES.md` gewinnt für Simulationsregeln.
4. `docs/STATUS.md` gewinnt für aktuellen realen Implementierungsstand.
5. `docs/NEXT_STEPS.md` gewinnt für den nächsten Arbeitsblock.
6. `AGENTS.md` gewinnt für Arbeitsprozess und Agentenpflichten.
7. `README.md` ist Orientierung, nicht Detailautorität.
8. Ältere Notizen/Handoffs sind Hintergrund, keine Wahrheit.

### 7.3 Update-Regel

- Codeänderungen, die Verhalten, Architektur oder nächste Arbeit verändern, aktualisieren mindestens `STATUS.md` oder `NEXT_STEPS.md`.
- Architekturentscheidungen gehen nach `DECISIONS.md`.
- Sim-Regeln gehen nach `SIM_RULES.md`.
- Bugs, Debug-Befunde und Review-Findings gehen nach `FINDINGS.md`.
- Das Planungs-Canvas wird nur aktualisiert, wenn sich die Richtung wirklich ändert.
...
## 9. Empfohlene Slices – konsolidierte v0.2-Roadmap

Die Gegenpläne zeigen: Mein ursprünglicher Slice 1 war noch zu groß, weil er Hex-Grundlage, statischen Organismus und Rendering bereits koppelte. Für den Neustart ist die bessere Reihenfolge: **erst Hex-Topologie beweisen, dann Körper, dann Lab, dann Energie, dann Wachstum.**
...
- Ordnerstruktur nach v0.2 anlegen
- `AGENTS.md`, `CLAUDE.md`, `README.md` und kanonische Docs erstellen
- Config-Resource-Skelette anlegen
- minimaler eigener Test-Runner oder Headless-Validator-Skelett
- minimal lauffähige Lab-Szene ohne Simulationskomplexität
...
- Test-/Validator-Skelett läuft headless
- kein Wachstum, keine Energie, keine Population, kein Rendering-Polish
...
- Distanz
- Boundary-Erkennung
- Connectedness-Prüfung

Akzeptanz:

- Tests für Nachbarn, Distanz, bekannte Formen, Boundary und Connectedness
- keine Godot-Node-Abhängigkeit im Hex-Kern
- Sim kennt keine Pixelorientierung
- Renderer-Orientierung bleibt Projektion

### Slice 2 – Genom und Startkörper

Ziel:

- kleines typisiertes `Genome` / `GenomeExpression`
- deterministische `GenomeFactory`
- einfache `OrganismBody`-/`BodyFactory`
- Pflichtzellen: `energy_core`, `photosynthesis`, `reproduction`, `wall`
- optionaler Kandidat `cytoplasm/body` bleibt offen, wird aber nicht automatisch eingeführt

Akzeptanz:

- gleicher Seed erzeugt denselben Startkörper
- 20–30 Seeds erzeugen unterscheidbare, valide Körper
- Körper sind connected
- keine doppelten Hex-Koordinaten
- Pflichtfunktionen existieren
- Renderer ist noch nicht notwendig
...
- einfacher `HexOrganismRenderer` über `_draw()` aus Snapshot/State
- Seed-Navigation
- Debug-Overlay für Koordinaten, Boundary, Zellfunktionen
- Inspector-konfigurierbare Config-Resources

Akzeptanz:

- 20–30 Seeds visuell schnell prüfbar
- Renderer erzeugt keine Simulationsentscheidungen
- UI verändert keine gespeicherten `.tres`-Assets unbeabsichtigt
- einfache Hex-Farben und Outlines reichen; keine organische Grafik
...
- Organismus besitzt globalen Energiepool
- Zellen haben einfache Maintenance-Kosten
- Energie-Core ist zunächst Speicher-/Bilanzanker, keine lokale Diffusionssimulation

Akzeptanz:

- Energie läuft deterministisch
- keine negativen/NaN-/Infinity-Werte
- bekannte Testkörper ergeben erwartbare Energiebilanz
...
- Reproduction-Zellen erzeugen Wachstumsladung
- neue Zellen entstehen an Frontier-Hexen
- Growth-Scoring berücksichtigt Genom, Oberfläche, Kompaktheit, Nachbarzahl und Zelltyp-Balance
- maximal begrenzte Growth-Events pro Tick

Akzeptanz:

- Wachstum bleibt connected
- neue Zellen entstehen nur adjacent zu bestehendem Körper
- keine Doppelbelegung
- Seed-Verhalten bleibt deterministisch
- Debug zeigt, warum eine Wachstumsentscheidung getroffen wurde
...
> Der Neustart sollte nicht versuchen, sofort „lebendige Zellen“ schön darzustellen. Er sollte zuerst beweisen, dass ein Organismus als deterministischer Hex-Zellkörper existieren, sichtbar geprüft werden kann, Energie erzeugt und später kontrolliert wächst – sauber getrennt, konfigurierbar, validierbar und für Agenten gut führbar.

Minimaler erster Meilenstein:

```text
Ein einzelner Hex-Organismus aus Basiszellen,
der deterministisch aus Seed/Config entsteht,
zusammenhängend ist,
einfach im Lab gerendert wird
...
## 18. Triaden-Synthese aus Claude Code und Codex

### 18.1 Was übernommen wird

Aus Claude Code übernehmen:

- Zellfunktionen als Daten-Tags statt Klassenhierarchie.
- explizite Future-Scope-Liste für Bewegung, Sicht, Verdauung, Kampf und organisches Rendering.
- kurzer `CLAUDE.md`-Review-Fokus, aber kein zweites Regelwerk.
- ADR-Pflicht für echte Architekturentscheidungen.
- Performance-Budgets als frühe Denkweise, aber nicht als Render-Overengineering in Slice 0.
- Inspector-Konfigurierbarkeit über `Resource` + `@export_range`.

Aus Codex übernehmen:

- stärker gesplittete Slice-Reihenfolge.
- `SIM_RULES.md` statt separatem Zell-Dokument.
- `FINDINGS.md` statt reinem `BUGS.md`.
- eigener schlanker Test-Runner/Headless-Validator statt sofortiger Test-Framework-Bindung.
- Boundary als abgeleitete Topologie, nicht automatisch als `wall`.
- globale Organismusenergie in v1, lokale Diffusion erst später.
- Rendering zuerst funktional per `_draw()`, MultiMesh erst nach Messung/Skalierungsbedarf.

### 18.2 Was bewusst nicht übernommen wird

Nicht übernommen:

- Pflicht zu exakt zwei Autoloads ab Slice 0.
- MultiMesh/Shader als Startpfad.
- echte Mutation bereits in frühen Slices.
- echte Tochterorganismen vor stabilem Wachstum.
- `ORCHESTRATOR_RESULT` als verpflichtendes Repo-Format. Es bleibt optional für externe Orchestrator-Läufe.
- separate `BUGS.md`, solange `FINDINGS.md` reicht.

### 18.3 Entscheidungen vor Slice 0

Vor Slice 0 sollten nur diese Punkte festgelegt werden:

1. **Projektname:** Arbeitsname `Baktorium`, Repo z. B. `baktorium`.
2. **Doku-Set:** v0.2-Dokumentenset übernehmen.
3. **Sim-Wahrheit:** Zellen und Organismen sind Daten, keine Nodes.
4. **Hex-Koordinaten:** axial `q,r`; cube nur abgeleitet.
5. **Orientierung:** Sim bleibt orientation-agnostisch; `pointy`/`flat` ist RenderConfig.
6. **Weltmodell:** Organismuslokales Hex-Body-Modell plus `WorldState` ab Tag 1; zunächst nur ein Organismus im Lab.
7. **Energie:** globaler Organismuspool in v1; keine lokale Diffusion.
8. **Boundary:** abgeleitete Topologie; `wall` ist Zellfunktion, nicht automatisch jede Außenkante.
9. **Renderer:** `_draw()` für Lab-v1; MultiMesh als späterer Performance-Spike.
10. **Testpfad:** eigener Headless-Validator/Test-Runner ab Slice 0.

### 18.4 Meine kritische Bewertung

Der Codex-Plan ist als Arbeitsroadmap stärker, weil er den Start konsequenter entkoppelt: Hex-Kern vor Körper, Körper vor Rendering, Rendering vor Energie, Energie vor Wachstum.

Der Claude-Plan ist als Architekturwächter stärker, weil er klarer formuliert, wo die spätere Komplexität andocken darf: Daten-Tags, ADRs, Reviewer-Rolle, Resource-Konfiguration und Performance-Budgets.

Die beste Fassung ist daher nicht entweder/oder, sondern:

```text
Codex-Roadmap als Slice-Grundlage
+
Claude-Architekturregeln als Guardrails
+
ChatGPT/Lyra-Fokus auf Scope-Schutz und Emergenz-Disziplin
```

### 18.5 Nächster Codex-Auftrag – aktualisierte Fassung

```text
Bitte setze für den Baktorium-Neustart ausschließlich Slice 0 um.

Ziel:
- Neues Godot-Projekt-Fundament unter dem Arbeitsnamen Baktorium anlegen.
- Ordnerstruktur gemäß Planungsdokument v0.2 erstellen.
- AGENTS.md, CLAUDE.md, README.md sowie docs/ARCHITEKTUR.md, SIM_RULES.md, DECISIONS.md, STATUS.md, NEXT_STEPS.md, FINDINGS.md anlegen.
- Config-Resource-Skelette vorbereiten.
- Einen minimalen Headless-Test-Runner oder Validator-Skeleton anlegen.
- Eine minimal lauffähige Lab-Szene erstellen, die noch keine echte Simulation enthält.
...
- kein MultiMesh/Shader-Spike
- keine Mutation
- keine Bewegung

Arbeitsregeln:
- Lies das Planungsdokument vollständig.
- Arbeite nach AGENTS.md.
- Halte Szenen, UI, Debug und Renderer frei von Simulationsentscheidungen.
- Änderungen so groß wie möglich und so klein wie nötig schneiden.
- Keine Auto-Commits, kein Push, keine PR ohne Freigabe.
...
- Abschluss mit geänderten Dateien, Validierung, offenen Risiken, Doku-Sync und Commit-Vorschlag.
```
